...
public class SmitPageTag extends TagSupport {
	...
	public int doStartTag() {  
	   try{  
	    ...
	    out.print("第 " + pager.getPageIndex() + " 页/共 " + (pager.getPageCount()) + " 页");  
	    ...
	    if(pager.getPageIndex() != 1) {  
	     ...
	     out.print("[<a href='" + pager.getUrl() + "&cpage=" + (pager.getPageIndex() - 1) + "'>上一页</a>]");  
	    }  
	    ...
	    for(int i = pager.getPageIndex() - 3; i <= pager.getPageIndex() + 3; i++) {  
	     ...
	     if(i == pager.getPageIndex()) {  
	      ...
	      out.print("[<a href='" + pager.getUrl() + "&cpage=" + i + "'> " + i + " </a>]");  
	     }  
	    }  
	    ...
	    if (pager.getPageIndex() != pager.getPageCount()  
	      && pager.getPageCount() != 0) {  
	     out.print("[<a href='" + pager.getUrl() + "&cpage=" + (pager.getPageIndex() + 1) + "'>下一页</a>]");  
	     out.print("[<a href='" + pager.getUrl() + "&cpage=" + (pager.getPageCount()) + "'>最末页</a>]");  
	    }  
	    out.flush();  
