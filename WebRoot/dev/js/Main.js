...
function setQueryOperator(I,Operator)
{
    ...
    if(!flag)
    {
        if(typeof(CustomFieldName) != "undefined")
        {
            for(var i = 0; i < CustomFieldName.length; i++)
            {
                var field = CustomFieldName[i];
                if(FieldName == field)
                {
                    delAllItem(xajax.$("Operator"+I));
                    addOptionByTextValue(xajax.$("Operator"+I), FieldOperationTypeText[0].split(','),FieldOperationTypeValue[0].split(','));
                    break;
                }
            }
        }
    ...
    else
    {
        ...
        if(typeof(CustomField) != "undefined")
        {
            for(var field in CustomField)
            {
                if(FieldName == field)
                {
                    tdObj.innerHTML = createSelect("Value" + I,CustomField[field]['key'],CustomField[field]['value'],FieldValue);
                    flag = true;
                    break;
                }
            }
